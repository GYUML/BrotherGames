...
{
    public class AdventureGameLogic : MonoBehaviour
    {
        Action<int[,]> onUpdateGameBoard;
        Action<int, int> onUpdateScore;
        Action<int> onUpdateRemainCount;
        Action<float, float> onSetTimer;
        Action<int, int> onEndGame;
        ...
        private void Update()
        {
            if (playingGame && remainSeconds > 0)
            {
                remainSeconds -= Time.deltaTime;
                if (remainSeconds <= 0)
                {
                    onEndGame?.Invoke(targetScore, score);
                }
            }
        }

        public void SetGameBoardCallBack(Action<int[,]> onUpdateGameBoard, Action<int, int> onUpdateScore, Action<int> onUpdateRemainCount, Action<float, float> onSetTimer, Action<int, int> onEndGame, Action<int, int> onAcquire)
        {
        ...
        public void StartGame(int rowSize, int columnSize, int targetScore)
        {
            nowStage = 1;
            playingGame = true;
        ...
        {
            score += add;
            onUpdateScore?.Invoke(targetScore, score);
